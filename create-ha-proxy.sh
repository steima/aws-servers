...
SERVER01_PRIVATE_DNS="${PRIVATE_DNS}"
...
scp_file ../lottery/target/lottery-0.0.1-SNAPSHOT.jar "${PUBLIC_DNS}" /home/ec2-user/
execute_ssh "${PUBLIC_DNS}" "java -Dspring.datasource.url=jdbc:mysql://${DATABASE_PRIVATE_DNS}:3306/app_lottery?autoReconnect=true -jar /home/ec2-user/lottery-0.0.1-SNAPSHOT.jar > /home/ec2-user/spring.log &"
SERVER02_PRIVATE_DNS="${PRIVATE_DNS}"

# haproxy server
source ./create-server.sh
wait_for_ssh "${PUBLIC_DNS}"
execute_ssh "${PUBLIC_DNS}" "sudo yum -y install haproxy"
scp_file haproxy.cfg "${PUBLIC_DNS}" /home/ec2-user/
execute_ssh "${PUBLIC_DNS}" "echo server web01 ${SERVER01_PRIVATE_DNS}:5000 >> /home/ec2-user/haproxy.cfg"
execute_ssh "${PUBLIC_DNS}" "echo server web02 ${SERVER02_PRIVATE_DNS}:5000 >> /home/ec2-user/haproxy.cfg"
execute_ssh "${PUBLIC_DNS}" "sudo mv /home/ec2-user/haproxy.cfg /etc/haproxy/haproxy.cfg"
execute_ssh "${PUBLIC_DNS}" "sudo /etc/init.d/haproxy start"
